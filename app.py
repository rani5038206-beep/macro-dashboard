...
def safe_signal(col, invert=False):
    ...
    return np.where(weekly[col] > ma, -1 if invert else 1, 1 if invert else -1)

signal["SPX"] = safe_signal("SPX")
signal["DXY"] = safe_signal("DXY", True)
signal["VIX"] = safe_signal("VIX", True)
signal["US10Y"] = safe_signal("US10Y", True)
...
mom["NIFTY"] = safe_mom("NIFTY")
mom["BANK"] = safe_mom("BANK")
mom["IT"] = safe_mom("IT")
...
with col3:
    st.metric("Last Updated", df.index[-1].strftime("%d %b %Y"))
